...
public class BallController : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject effectTimeViewGameObject;
    [SerializeField]
    private TextMeshProUGUI effectTimeView;
    [SerializeField]
    private Texture[] skins = new Texture[2];
    private Material LeftWall, RightWall;
    [SerializeField]
    private AudioSource DestroyCubeSound;
    [SerializeField]
    private MeshRenderer LeftWallRdr, RightWallRdr;
    ...
    private IEnumerator timeEstimated()
    {
        ...
        effectTimeView.text = LocalizeManager.GetLocalizedString(LocalizeManager.TapHereForEffect, true);
    }
    ...
    {
        if (timeForEffect == 0 && game.gameState == Game.GameState.Running)
        {
    ...
    {
        currentTransform.Rotate(Vector3.left * valueManager.GetCubeSpeed(false) * 50f);
    }
    ...
    {
        if (game.gameState == Game.GameState.Running && canMove)
            Move(-1f);
    }
