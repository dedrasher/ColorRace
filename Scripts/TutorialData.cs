...
public class TutorialData
{
    ...
    public bool ShopAlertQuery = true;
    ...
    public string GetGameTutorialText(GameTutorialType gameTutorial)
    {
        if (gameTutorial == GameTutorialType.Control)
            return LocalizeManager.CurrentLanguage == LocalizeManager.Language.Russian ? "Для перемещения налево нажимать на левую часть экрана, направо - на правую" :
                "To move to the left, click on the left side of the screen, to the right - on the right";
        else
            return LocalizeManager.CurrentLanguage == LocalizeManager.Language.Russian ? "Цель игры -разбивать кубики в цвет бортиков стен" :
    ...
    public string ShopAlertText
    {
        get
        {
            return LocalizeManager.CurrentLanguage == LocalizeManager.Language.Russian ? "Перейдите в магазин для покупки своего первого скина!" :
